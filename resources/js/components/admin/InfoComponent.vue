...
export default {
    ...
    methods: {
        getAllData() {
            axios.get(base_site + '/dashboard/info/get?q=' + this.q).then(res => {
                this.countries = res.data;
            });
        },
        getResults(page = 1) {
            axios.get(base_site + '/dashboard/info/get?page=' + page + '&q=' + this.q)
                .then(response => {
        ...
        save() {
            this.form.post(base_site + '/dashboard/info/save').then(({ }) => {
                Toast.fire({
                    icon: 'success',
        ...
        showeditDialog(id) {
            this.form.clear();
            this.form.reset();
            axios.get(base_site + '/dashboard/info/' + id + '/get').then(({ data }) => {
                this.form.fill(data);
            })
            $('#addCountry').modal('show');
        },
        update() {
            this.form.put(base_site + '/dashboard/info/update').then(() => {
                Toast.fire({
                    icon: 'success',
        ...
        deleteCountry(id) {
            Swal.fire({
                ...
                if (result.value) {
                    axios.delete(base_site + '/dashboard/info/' + id + '/delete').then(({ data }) => {
                        Fire.$emit('DeleteCountry');
                        Swal.fire(
                            'حذف!',
                            'تم الحذف بنجاح .',
