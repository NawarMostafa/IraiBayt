...
export default {
    ...
    methods: {
        ...
        getAllData() {
            axios.get(base_site + '/dashboard/categories/get?q=' + this.q).then(res => {
                this.categories = res.data;
            });
        },
        getResults(page = 1) {
            axios.get(base_site + '/dashboard/categories/get?page=' + page + '&q=' + this.q)
                .then(response => {
        ...
        save() {
            this.form.post(base_site + '/dashboard/categories/save').then(({ }) => {
                Toast.fire({
                    icon: 'success',
        ...
        showeditDialog(id) {
            ...
            this.form.clear();
            this.form.reset();
            axios.get(base_site + '/dashboard/categories/' + id + '/get').then(({ data }) => {
                this.form.fill(data);
                if (data.img == null) {
                    this.form.img = ''
                }
        ...
        update() {
            this.form.put(base_site + '/dashboard/categories/update').then(() => {
                Toast.fire({
                    icon: 'success',
        ...
        deleteCategory(id) {
            Swal.fire({
                ...
                if (result.value) {
                    axios.delete(base_site + '/dashboard/categories/' + id + '/delete').then(({ data }) => {
                        Fire.$emit('DeleteCountry');
                        Swal.fire(
                            'حذف!',
                            'تم الحذف بنجاح .',
