            ...
            <div class="card">
                ...
                <div class="card-body table-responsive">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr v-for="currancy in currancies.data" v-bind:key="currancy.id">
                            <td>{{ currancy.name }}</td>
                            <td>{{ currancy.short_name }}</td>
                            <td>{{ currancy.active | active }}</td>
...
export default {
    ...
    methods: {
        ...
        getAllData() {
            axios.get(base_site + '/dashboard/currancies/get?q=' + this.q).then(res => {
                this.currancies = res.data;
            });
        },
        getResults(page = 1) {
            axios.get(base_site + '/dashboard/currancies/get?page=' + page + '&q=' + this.q)
                .then(response => {
        ...
        save() {
            this.form.post(base_site + '/dashboard/currancies/save').then(({ }) => {
                Toast.fire({
                    icon: 'success',
        ...
        showeditDialog(id) {
            ...
            this.form.clear();
            this.form.reset();
            axios.get(base_site + '/dashboard/currancies/' + id + '/get').then(({ data }) => {
                this.form.fill(data);
        ...
        update() {
            this.form.put(base_site + '/dashboard/currancies/update').then(() => {
                Toast.fire({
                    icon: 'success',
        ...
        deleteCurrancy(id) {
            Swal.fire({
                ...
                if (result.value) {
                    axios.delete(base_site + '/dashboard/currancies/' + id + '/delete').then(({ data }) => {
                        Fire.$emit('DeleteCountry');
                        Swal.fire(
                            'حذف!',
                            'تم الحذف بنجاح .',
