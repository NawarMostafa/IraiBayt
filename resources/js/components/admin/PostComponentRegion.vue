            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="justify-content-between d-flex">
                        <span> الإعلانات الموجودة ضمن المنطقة</span>
                        <abbr title="">
...
export default {
    ...
    methods: {
        ...
        getAllData(region) {
            axios.get(base_site + '/dashboard/posts/' + region + '/getregion').then(res => {
                this.posts = res.data;
            });
        },
        getResults(region, page = 1) {
            axios.get(base_site + '/dashboard/posts/' + region + '/getregion?page=' + page).then(res => {
                this.posts = res.data;
            });
        },
        showDialog() {
